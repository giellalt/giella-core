...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="outFile" select="concat($SLANG,'-',$TNUM)"/>
  ...
  <xsl:template match="/" name="main">
	...
	<xsl:variable name="tcounter" select="count(.//dg[./@xml:lang=$TLANG]/d)"/>
<!--
	<xsl:if test="$tcounter=0">
	  <xsl:message terminate="yes">
	    <xsl:value-of select="concat('No d-element found. Please check the xml:lang attribute on dg-elements!', $nl)"/>
	  </xsl:message>
	</xsl:if>
-->
    ...
    <!-- The translate is used to escape special characters such as ! or : appearing in lemmas or translations -->
	  ...
	  <xsl:if test="$TNUM='first'">
	    <xsl:value-of
		select="concat($cl,':',replace(translate(normalize-space(./mg[1]/dg[1]/d[1]),
			' ','_'), '([!:;])', '%$1') , ' # ;',$nl)"/>
	  </xsl:if>
