...
<!--+
    ...
    | e.g.
    | java -Xmx2048m net.sf.saxon.Transform -it:main _six scripts/extract_l-t-pos-dix.xsl SLANG=sme TLANG=smj TNUM=first  GTHOME=$GTHOME
    |
    | - parameter to adjust:
    |     (a) the script being GTHOME/words/dicts/scripts
    |     (b) the inDir being GTHOME/words/dicts/SLANGTLANG/src
    |     (c) the outDir being GTHOME/words/dicts/SLANGTLANG/bin
    |
...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="GTHOME" select="'yyy'"/>
  <xsl:param name="inDir" select="concat($GTHOME,'/words/dicts/',$SLANG,$TLANG,'/src')"/>
  <xsl:param name="inFile" select="concat('*_',$SLANG,$TLANG,'.xml')"/>
  <xsl:param name="outDir" select="concat($GTHOME,'/words/dicts/',$SLANG,$TLANG,'/bin')"/>
  <xsl:param name="outFile" select="concat($SLANG,$TLANG,'-pos-',$TNUM)"/>
  ...
  <xsl:template match="/" name="main">
    ...
    <xsl:result-document href="{$outDir}/{$outFile}.txt" format="txt">
	...
	<xsl:for-each select=".//e">
	  ...
	  <xsl:variable name="clpos" select="./lg/l/@pos"/>

	  <xsl:if test="$TNUM='first'">
	    <xsl:value-of select="concat($cl,' ',$clpos,' : ')"/>
	    <xsl:value-of select="translate(normalize-space(./mg[1]/tg[1][./@xml:lang=$TLANG]/t[1]),' ','_')"/>
	    <xsl:value-of select="concat(' ', ./mg[1]/tg[1][./@xml:lang=$TLANG]/t[1]/@pos, $nl)"/>
	  </xsl:if>
	  <xsl:if test="$TNUM='all'">
	    <xsl:for-each select=".//tg[./@xml:lang=$TLANG]/t">
	      <xsl:value-of select="concat($cl,' ',$clpos,' : ')"/>
	      <xsl:value-of select="translate(normalize-space(.), ' ', '_')"/>
	      <xsl:value-of select="concat(' ', @pos, $nl)"/>
	    </xsl:for-each>
	  </xsl:if>
	</xsl:for-each>
