...
- `pkg-version.json`: Package version number (from AC_INIT in configure.ac)
...
- `hyphenator-version.json`: Hyphenator version number (from HYPHVERSION in configure.ac)

### Grammar checker-related

- `gramcheck-version.json`: Grammar checker version number (from GRAMCHECKVERSION in configure.ac)
- `gramcheck-rules.json`: Number of error-detection rules in tools/grammarcheckers/grammarchecker.cg3

### TTS text processing-related

- `tts-textproc-version.json`: TTS text processing version number (from TTSTEXTPROCVERSION in configure.ac)
...
- `make-version-json.sh`: Generates version badges (FST, speller, hyphenator, gramcheck, TTS textproc, etc.)
  - Usage: `make-version-json.sh INPUTDIR VARNAME [--label LABEL]`
  - VARNAME can be: FST, SPELLER, HYPH, GRAMCHECK, TTSTEXTPROC, etc.
- `make-gramcheck-rules-json.sh`: Generates grammar checker error-detection rule count badge
  - Counts rules via `count-gramcheck-rules.sh`, which looks for rules of the
    form `RULETYPE:rule-name (&errortag) ...` in
