...
/usr/local/bin/hfst-summarize "$FSAFILE" -v |\
    grep -F -A 1 "sigma set:" |\
    tail -n 1 |\
    tr , '\n' |\
    /usr/bin/sed -e 's/^ //' |\
    grep -E -v '^[+{@}^]' > "$REGEXFILE".sigma.nfc
/opt/homebrew/Cellar/icu4c/72.1/bin/uconv -x any-nfd "$REGEXFILE".sigma.nfc > "$REGEXFILE".sigma.nfd
paste "$REGEXFILE".sigma.nfc "$REGEXFILE".sigma.nfd |\
    /opt/homebrew/bin/awk '$1 != $2 {printf("%s (->) %s,\n", $1, $2);
    printf("%s (->) ", $1);
